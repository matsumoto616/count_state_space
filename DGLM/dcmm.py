...
from DGLM.utils import ZeroAndPlus_AlphaAndBeta, ZeroAndPlus_MeanAndCov
...
class BinaryPoissonDCMM:
    ...
    def theta_predict_step(
        ...
        theta_zero_pred = self.bl_dglm.theta_predict_step(
            theta_zero_filt, G_zero, W_zero
        )
        theta_plus_pred = self.pl_dglm.theta_predict_step(
            theta_plus_filt, G_plus, W_plus
        )

        return ZeroAndPlus_MeanAndCov(theta_zero_pred, theta_plus_pred)

    def lambda_predict_step(
        self, theta_pred: ZeroAndPlus_MeanAndCov, F_zero: np.ndarray, F_plus: np.ndarray
    ) -> ZeroAndPlus_MeanAndCov:
    ...
    def calc_alpha_beta(
        self, lambda_pred: ZeroAndPlus_MeanAndCov
    ...
    def lambda_filter_step(
        self, alpha_beta: ZeroAndPlus_AlphaAndBeta, y_obs: int
    ) -> ZeroAndPlus_MeanAndCov:
        alpha_beta_zero, alpha_beta_plus = alpha_beta.zero, alpha_beta.plus
    ...
    def theta_filter_step(
        ...
        theta_zero_filt = self.bl_dglm.theta_filter_step(
            theta_zero_pred, lambda_zero_pred, lambda_zero_filt, F_zero
        )
        if lambda_plus_filt is not None:
            theta_plus_filt = self.pl_dglm.theta_filter_step(
                theta_plus_pred, lambda_plus_pred, lambda_plus_filt, F_plus
            )
