...
def make_weekly_data(start_date, end_date, seed=0):
    ...
    df_data["z"] = df_data["y_weekly"].map(lambda x: int(bool(x)))
    df_data["y_raw"] = df_data["y_weekly"]
...
def make_weekly_and_yearly_data(start_date, end_date, seed=0):
    ...
    df_data["z"] = df_data["y_yearly"].map(lambda x: int(bool(x))) * df_data[
        "weekday"
    ].map(lambda x: int(bool(weekday_range[x])))
...
def make_random_data(
    ...
    value += rng.integers(
        -weekday_range[weekday], weekday_range[weekday], endpoint=True
    )
